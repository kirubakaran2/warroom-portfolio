...
/**
 * Autoscroll
 * A horizontally auto‑scrolling carousel of images.
 * The images are duplicated to create a seamless infinite loop.
 */
export const Autoscroll = (props: {
    images: { src: string; alt: string }[];
    /**
     * Duration of one full scroll loop in seconds.
     * Adjust to control scroll speed.
     */
    duration?: number;
}) => {
    const { images, duration = 15 } = props;

    // Duplicate the images array so the scroll appears continuous
                ...
                {allImages.map((img, idx) => (
                    <div key={idx} className="flex-shrink-0 pr-4">
                        <img
                            src={img.src}
                            alt={img.alt}
                            className="h-60 w-auto rounded-lg object-cover shadow-lg border border-white/10"
                        />
                    </div>
                ))}
            </motion.div>
...
class Media {
    ...
    onResize(
        ...
        const targetWidth = isMobile ? 350 : 1200;
        const targetHeight = isMobile ? 233 : 800; // Maintain approx 3:2 aspect ratio
        const padding = isMobile ? 0.5 : 1;

        this.plane.scale.y =
            (this.viewport.height * (targetHeight * this.scale)) / this.screen.height;
        this.plane.scale.x =
            (this.viewport.width * (targetWidth * this.scale)) / this.screen.width;
    ...
    onResize() {
        ...
        if (this.medias) {
            this.medias.forEach((media) =>
                media.onResize({ screen: this.screen, viewport: this.viewport, bend: effectiveBend }),
            );
        }
    }
