...
import { CircularRevealHeading } from "./components/ui/circle"
import { Instagram, Youtube } from "lucide-react"
import { PiDiscordLogoFill } from "react-icons/pi"
import { FaXTwitter } from "react-icons/fa6"
import { GiKnifeFork, GiRocket, GiGameConsole } from "react-icons/gi"
import { useGSAP } from '@gsap/react'
import gsap from 'gsap'
import { SplitText } from 'gsap/SplitText'
import zeus2 from "../public/zeus2.jpeg"
import zeus3 from "../public/zeus3.jpeg"
import zeus from "../public/Zeus Ancient Sculpture.jpg"
...
const fragmentShader = `
  ...
  vec4 cppn_fn(vec2 coordinate, float in0, float in1, float in2) {
    ...
    * buf[3]
    + mat4(vec4(3.4992442, -196.91893, -8.923708, 2.8142626), vec4(3.4806502, -3.1846354, 5.1725626, 5.1804223), vec4(-2.4009497, 15.585794, 1.2863957, 1.0252278), vec4(-71.25271, -62.441242, -8.138444, 0.50670296))
    * buf[4]
  ...
  // Gold tone base color
  vec3 gold = vec3(1.0, 0.843, 0.0); // RGB for gold (like #FFD700)

  // Add subtle shimmer animation for realism
  float shimmer = 0.2 + 0.8 * abs(sin(iTime * 0.8 + uv.x * 4.0 + uv.y * 4.0));

  // Mix gold tone with shader pattern
  vec3 finalColor = mix(gold * 0.6, gold * 1.4, shimmer) * cppnColor.rgb;
...
const HoverableIcon: React.FC<HoverableIconProps> = ({ href, label, children }) => {
  ...
  return (
    <div
      className="relative hover-target"
    ...
    >
      ...
      {hover && (
        <motion.span
          ...
          className="absolute left-1/2 -translate-x-1/2 text-xs bg-black/50 text-white/90 rounded-md px-2 py-1 mt-1 backdrop-blur-sm"
        >
          {label}
        </motion.span>
      )}
...
export default function HeroOne() {
  ...
  const iconMap: Record<string, React.ReactNode> = {
    "Think Sharp": <GiKnifeFork className="text-red-400 text-2xl" />,
    "Move Fast": <GiRocket className="text-blue-400 text-2xl" />,
    "Own the Game": <GiGameConsole className="text-green-400 text-2xl" />,
  }
  ...
  return (
    <section ref={sectionRef} className="relative h-screen w-screen overflow-hidden">
      ...
      <div className="relative h-full w-full flex">
        ...
        <div className="flex-1 flex flex-col justify-center pl-12 sm:pl-16 md:pl-20 lg:pl-24 xl:pl-32 pr-6 sm:pr-8 md:px-12 py-24">
          <div className="max-w-2xl">
            {/* Badge */}
            <div ref={badgeRef} className="inline-flex items-center px-4 py-2 rounded-full bg-gradient-to-r from-gray-800/80 to-black/80 border border-gray-600/50 mb-8 backdrop-blur-sm">
              <span className="w-2 h-2 bg-green-400 rounded-full mr-2 animate-pulse"></span>
              <span className="text-sm font-medium text-white/90 font font-light font-sanchez">For Relentless Minds</span>
              <span className=" font-sanchez ml-2 px-2 py-1 text-xs bg-yellow-500/90 text-black rounded-full font-bold font font-medium">
                Exclusive
              </span>
            </div>

            {/* Main Title */}
            <h1 ref={headerRef} className="text-4xl md:text-5xl lg:text-6xl font-bold text-white mb-6 leading-tight font-bitcount" style={{ fontFamily: "'Bitcount', sans-serif", fontWeight: 400 }}>
              Where Discipline
              <br />
            ...
            {/* Description */}
            <p ref={paraRef} className="text-lg md:text-xl text-gray-300/90 mb-8 max-w-2xl leading-relaxed font-sanchez font-medium tracking-wide">
              Train. Strategize. Execute. Gain the edge with live sessions, tactical insights,
              and field-tested systems built for the relentless.
            </p>
            ...
            <div ref={ctaRef} className="flex flex-col sm:flex-row gap-4 mb-12">
              <motion.a
                ...
                whileTap={{ scale: 0.95 }}
                href="#get-started"
                className="bg-gradient-to-r from-yellow-500 to-orange-500 text-black font-sanchez font-semibold py-3 px-8 rounded-lg text-center transition-all duration-300 hover:shadow-lg hover:shadow-yellow-500/25 font font-semibold"
              >
                Join Now
              </motion.a>
              <motion.a
                ...
                whileTap={{ scale: 0.95 }}
                href="#explore"
                className="border border-gray-500/50 text-white/90 font-sanchez font-semibold py-3 px-8 rounded-lg text-center transition-all duration-300 hover:bg-white/5 backdrop-blur-sm font font-semibold"
              >
                Explore More
              </motion.a>
            </div>
            ...
            {/* Micro Details */}
            <div className="flex flex-wrap gap-6 mb-8 font-bitcount" style={{ fontFamily: "'Bitcount', sans-serif",fontWeight: 200  }}>
              {["Think Sharp", "Move Fast", "Own the Game"].map((text) => (
                <motion.div
                  key={text}
                  onMouseEnter={() => setHovered(text)}
                  onMouseLeave={() => setHovered("")}
                  whileHover={{ scale: 1.05 }}
                  className="relative font-semibold text-lg text-gray-300/80 hover:text-white/90 transition-colors duration-300 hover-target select-none cursor-pointer font font-medium"
                >
                  {text}
                  {hovered === text && (
                    <motion.div
            ...
            <div className="flex space-x-6">
              <HoverableIcon href="https://www.instagram.com/ict_dinesh/" label="View Instagram">
                <Instagram size={22} />
              </HoverableIcon>

              <HoverableIcon href="https://x.com/ict_dinesh/status/1986090000153448690" label="View on X">
                <FaXTwitter size={22} />
              </HoverableIcon>

              <HoverableIcon href="https://discord.gg/qnpfuETS" label="Join Discord">
                <PiDiscordLogoFill size={22} />
              </HoverableIcon>

              <HoverableIcon href="https://www.youtube.com/@ict_dinesh" label="Watch on YouTube">
                <Youtube size={22} />
              </HoverableIcon>
            </div>
          </div>
        ...
        {/* Right Side - Circular Component */}
        <div className="flex-1 hidden lg:flex items-center justify-center pr-8 lg:pr-16 xl:pr-24">
          <div className="w-full max-w-md transform scale-95">
            <CircularRevealHeading
              items={items}
              centerText={
                <div className="text-lg font-bold text-gray-800/90 font font-bold">
                  WARROOM
                </div>
              }
              size="md"
